...
export default {
  ...
  async created () {
      try {
          const {data} = await axios.get('https://parkingapp.gigalixirapp.com/api/bookings/all', { headers: {"Authorization" : `Bearer ${localStorage.getItem('token')}`}})
  ...
  methods: {
      ...
      async btnCancelClick(bookingObj){
          try {
              const {data} = await axios.post('https://parkingapp.gigalixirapp.com/api/bookings/cancel', {id: bookingObj.id}, { headers: {"Authorization" : `Bearer ${localStorage.getItem('token')}`}})
          } catch(e) {
              alert('Error:'+ e)
          }
