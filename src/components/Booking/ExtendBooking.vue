...
export default {
  ...
  methods: {
    async extendBooking() {
      try{
        const {data} = await axios.post('https://parkingapp.gigalixirapp.com/api/bookings/update', {id: this.bookingDetails.id, start_time: this.bookingDetails.start_time, end_time: this.newEndTime, calc_criteria: this.bookingDetails.calc_criteria}, { headers: {"Authorization" : `Bearer ${localStorage.getItem('token')}`}})
        this.parkings = data
